import { Button, ConfirmModal, Modal } from "@savantly/sprout-ui";
import { create } from "lodash";
import React, { Component, createRef, ReactElement, Ref } from "react";
...
export class ModalProxy extends LifecycleLogging<OwnProps, OwnState> {
  ...
  showModalReact(options: any) {
    ...
    const modalProps = {
        ...options.props, isOpen: true, onDismiss: this.onReactModalDismiss
    };
    this.setState({
  ...
  showConfirmModal(options: any) {
      const confirmModalProps = {
        ...
        /** Modal content */
        body: (<div>
            <h3>{options.text}</h3><br/>{options.text2}
        </div>),
