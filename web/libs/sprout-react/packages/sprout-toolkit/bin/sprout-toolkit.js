...
const isLinkedMode = () => {
  ...
  if (path.basename(pwd) === 'sprout-toolkit') {
    return true;
  }

  try {
    const resolvedPath = path.resolve(`${__dirname}/../../../node_modules/@savantly/sprout-toolkit`);
...
const entrypoint = () => {
  ...
  // IF we have a toolkit directory AND linked sprout toolkit AND the toolkit dir is a symbolic lik
