        ...
        switch (_b.label) {
            ...
            case 1:
                ...
                filteredSuggestions = suggestions
                    ...
                    if (prefix) {
                        ...
                        if (!group.skipFilter) {
                            ...
                            if (group.prefixMatch) {
                                newGroup.items = newGroup.items.filter(function (c) { return (c.filterText || c.label).startsWith(prefix); });
                            }
                            else {
                                newGroup.items = newGroup.items.filter(function (c) { return (c.filterText || c.label).includes(prefix); });
                            }
                        }
                    ...
                    if (!group.skipSort) {
                        newGroup.items = sortBy(newGroup.items, function (item) { return item.sortText || item.label; });
                    }
                    return newGroup;
                })
