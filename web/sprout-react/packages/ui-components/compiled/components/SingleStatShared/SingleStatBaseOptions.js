...
function migrateFromAngularSinglestat(panel, prevOptions) {
    ...
    var mappings = convertOldAngularValueMapping(prevPanel);
...
export function sharedSingleStatMigrationHandler(panel) {
    ...
    if (previousVersion < 6.6 && fieldOptions) {
        ...
        if (fieldOptions && fieldOptions.override) {
            var _b = options.fieldOptions, override = _b.override, rest = __rest(_b, ["override"]);
    ...
    if (previousVersion < 7.0) {
        ...
        panel.fieldConfig = {
            ...
            overrides: fieldOptions && fieldOptions.overrides
                ? __spread(panel.fieldConfig.overrides, fieldOptions.overrides) : panel.fieldConfig.overrides,
        };
        if (fieldOptions) {
...
export function moveThresholdsAndMappingsToField(old) {
    ...
    return __assign(__assign({}, old), { fieldOptions: __assign(__assign({}, rest), { defaults: __assign(__assign({}, fieldOptions.defaults), { mappings: mappings,
...
export function convertOldAngularValueMapping(panel) {
    ...
    if (mappingType === 1) {
        for (var i = 0; i < panel.valueMaps.length; i++) {
            ...
            mappings.push(__assign(__assign({}, map), { id: i, type: MappingType.ValueToText }));
        }
    }
    else if (mappingType === 2) {
        for (var i = 0; i < panel.rangeMaps.length; i++) {
            var map = panel.rangeMaps[i];
            mappings.push(__assign(__assign({}, map), { id: i, type: MappingType.RangeToText }));
        }
    }
    return mappings;
}
