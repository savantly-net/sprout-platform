...
export var getAllFields = memoizeOne(function (row, getFieldLinks) {
    ...
    var fieldsMap = __spread(derivedFields, fields).reduce(function (acc, field) {
        ...
        if (acc[fieldHash]) {
            acc[fieldHash].links = __spread((acc[fieldHash].links || []), (field.links || []));
        }
        else {
            acc[fieldHash] = field;
        }
...
var parseMessage = memoizeOne(function (rowEntry) {
    ...
    var parsedFields = parser.getFields(rowEntry);
    var fields = parsedFields.map(function (field) {
