...
export const bundlePlugin = async ({ watch, production, outputFolder, minimize }: PluginBundleOptions) => {
  ...
  const webpackPromise = new Promise<void>((resolve, reject) => {
    if (watch) {
      ...
      compiler.hooks.done.tap('done', (stats: webpack.Stats) => {
        clearConsole();
        const json: any = stats.toJson(); // different @types/webpack between react-dev-utils and sprout-toolkit
