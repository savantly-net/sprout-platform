...
export const jestConfig = (baseDir: string = process.cwd()) => {
  ...
  if (deniedOverrides.length > 0) {
    console.error("\nsprout-toolkit doesn't support following Jest options: ", deniedOverrides);
...
/**
 * This will load the existing jest setup, or use the default if it exists
 */
export const loadJestPluginConfig = (baseDir: string = process.cwd()) => {
  const cfgpath = path.resolve(baseDir, 'jest.config.js');
  if (!fs.existsSync(cfgpath)) {
    const src = path.resolve(baseDir, 'node_modules/@savantly/sprout-toolkit/src/config/jest.plugin.config.local.js');
